    ...
    modalForm.addEventListener('submit', async (event) => {
        ...
        const response = await fetch('http://localhost:5700/api/'+activeSection, {
            method: 'POST',
            body: JSON.stringify({
                newsEvent: { title, type, description, date: new Date().toISOString(), image_url: `/${activeSection}/${image.files[0].name}` , actual_resource_url: `/${activeSection}/${actualResource.files[0].name}` },
